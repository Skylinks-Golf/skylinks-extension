(function SelectPiSnapshotUI() {
    ...
    // Today in local timezone (avoids UTC-offset drift from toISOString())
    const _now = new Date();
    const todayStr = `${_now.getFullYear()}-${String(_now.getMonth() + 1).padStart(2, '0')}-${String(_now.getDate()).padStart(2, '0')}`;

    // 2. Build overlay scaffold
    const overlay = document.createElement('div');
    overlay.id = 'sp-snapshot-overlay';
    ...
    card.innerHTML = `
<div style="background:#eeb02b;border-radius:14px 14px 0 0;padding:16px 24px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;">
  <div>
  ...
  <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
    ...
    <button id="sps-close"   style="background:#fff;color:#1c1b19;border:none;padding:6px 12px;border-radius:8px;font-size:20px;font-weight:700;cursor:pointer;line-height:1;">×</button>
  </div>
</div>
<div id="sps-progress-wrap" style="display:none;">
...
<div id="sps-body" style="padding:24px;">
  <div style="text-align:center;padding:48px;color:#64748b;font-size:14px;">Loading…</div>
    ...
    const post = (path, body) => fetch(path, { method: 'POST', headers: apiHeaders, body: JSON.stringify(body) })
        .then(r => { if (!r.ok) throw new Error(`${r.status} ${r.statusText} (${path})`); return r.json(); });

    // 6. destroyCharts — must be called before re-render and on close
    ...
    async function fetchAll(date) {
        ...
        // NOTE: If start===end returns zero data, the API may treat it as a zero-length window.
        // In that case set end to the next day:
        //   end: new Date(new Date(date).getTime() + 86400000).toISOString().slice(0, 10)
        // and add a comment documenting the off-by-one behaviour.
        ...
        if (errors.length) {
            $('sps-errors').innerHTML = '<div style="padding:0 24px 4px;">' +
                errors.map(e =>
        ...
        // Log first row of each response so field names are visible in DevTools Console
    ...
    function render({ summary, dispensers, byLocation, byHour, byBucket }) {
        ...
        // Balls dispensed: Summary.model[0].totalBallsDispensed
        ...
        // Bucket totals: TotalsByBucketSize.models → {bucketSize, count, amount, size}
...
<div id="sps-bucket-grid">
  ...
  <div style="background:#f8fafc;border-radius:12px;padding:20px;border:1px solid #e2e8f0;">
    ...
    <div id="sps-bucket-table"></div>
        ...
        // --- Station color map ---
        // BallsDispensedByLocation returns {key:"LEFT", value:n} — match colors by station name.
        ...
        // --- Charts 1 & 2: BallsDispensedByLocation → {key, value} ---
        ...
        if (locModels.length) {
            ...
            state.charts.donut = new Chart($('sps-donut'), {
                ...
                data: {
                    labels: locLabels,
                    datasets: [{ data: locVals, backgroundColor: locColors, borderWidth: 2, borderColor: '#f8fafc' }],
                },
                options: {
                    responsive: true,
                    ...
                    plugins: {
                        ...
                        tooltip: {
                            callbacks: {
                                label: ctx => {
                                    const pct = locTotal > 0 ? (ctx.parsed / locTotal * 100).toFixed(1) : '0.0';
                                    return ` ${fmtNum(ctx.parsed)} balls (${pct}%)`;
                                },
                            },
                        },
                    },
            ...
            state.charts.stationBar = new Chart($('sps-station-bar'), {
                ...
                data: {
                    labels: locLabels,
                    datasets: [{ data: locVals, backgroundColor: locColors, borderRadius: 6 }],
                },
                options: {
                    responsive: true,
                    maintainAspectRatio: false,
        ...
        // --- Chart 3: BallsDispensedByHour → {key:"05AM", value:195} ---
        // Always single-series combined totals. Parse "05AM"/"12PM" to 0-23 integer for ordering.
        ...
        if (bucketModels.length) {
            ...
            state.charts.bucketBar = new Chart($('sps-bucket-bar'), {
                ...
                options: {
                    ...
                    plugins: { legend: { display: false } },
                    scales: {
                        x: { beginAtZero: true, ticks: { font: { size: 11 } }, grid: { color: '#e2e8f0' } },
  ...
  <td style="${TD}text-align:right;">${fmtMoney(totalBucketRev)}</td>
    ...
    // 9. load — resets body, destroys charts, fetches, then renders
    ...
    // 11. Initial load with today's date
    load(todayStr);
